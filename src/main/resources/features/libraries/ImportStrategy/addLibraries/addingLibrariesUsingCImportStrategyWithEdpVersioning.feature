Feature: Libraries provisioning using Import strategy with default versioning type
  ...
  Scenario Outline: Create Terraform library using Import strategy (gitlab)
    ...
    And User deletes "<applicationName>" codebase
    Examples:
      | applicationName | codebaseStrategy | gitServer | relativePath     | codeLanguage | buildTool | branchName | defaultBranchName | ciPipelineProvisioner | versioningType   |
      | terraform       | Import           | git-epam  | terraform-gitlab | Terraform    | Terraform | master     | master            | gitlab                | default          |
...
#
  ...
  Scenario Outline: Create Terraform library using Import strategy (github)
    ...
    Examples:
      | applicationName     | codebaseStrategy | gitServer | relativePath     | codeLanguage | buildTool | branchName | defaultBranchName | ciPipelineProvisioner | versioningType   |
      | terraform-terraform | Import           | github    | terraform-github | Terraform    | Terraform | master     | master            | github                | default          |
