Feature: Libraries provisioning using Import strategy with EDP versioning type
  ...
  Scenario Outline: Create Terraform library using Import strategy (gitlab)
    ...
    Examples:
      | applicationName | codebaseStrategy | gitServer | relativePath     | codeLanguage | buildTool | branchName | defaultBranchName | ciPipelineProvisioner | versioningType   | startVersion |
      | terraform       | Import           | git-epam  | terraform-gitlab | Terraform    | Terraform | master     | master            | gitlab                | edp              | 1.2.3        |
  ...
  Scenario Outline: Create Terraform library using Import strategy (github)
    ...
    Examples:
      | applicationName     | codebaseStrategy | gitServer | relativePath     | codeLanguage | buildTool | branchName | defaultBranchName | ciPipelineProvisioner | versioningType   | startVersion |
      | terraform-terraform | Import           | github    | terraform-github | Terraform    | Terraform | master     | master            | github                | edp              | 1.2.3        |
