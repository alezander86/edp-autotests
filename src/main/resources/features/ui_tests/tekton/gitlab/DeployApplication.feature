Feature: Deploy applications added using Create, Clone and Import strategies GitLab
  ...
  Scenario Outline: Deploy application added using Import strategy with default versioning type
    ...
    Examples:
      ...
      | java8-grd-gitlab-dep-ui | master            | java8_gradle_application | new-default   | java8-grd-deploy | sit       |

  Scenario Outline: Deploy application added using Clone strategy with EDP versioning type
    ...
    Examples:
      | applicationName      | defaultBranchName | codeLanguage          | newBranchName | pipelineName  | stageName |
      | go-gin-gitlab-dep-ui | master            | go_go_gin_application | new-edp       | go-gin-deploy | dev       |

  Scenario Outline: Deploy application added using Create strategy with EDP versioning type release branch
    Given User opens EDP Headlamp as default user
    When User opens EDP Components tab
    ...
    Examples:
      ...
      | js-antora-gitlab-dep-ui | master            | antora_application | release/7.5   | js-antora-deploy | dev       | release-7.5            |
